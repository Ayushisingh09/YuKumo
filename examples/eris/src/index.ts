...
client.on("voiceStateUpdate", (member, oldState) => {
  const guildId = member.guild.id ?? oldState?.guildId;
...
client.on("voiceServerUpdate", (data) => {
  const serverUpdate: VoiceServerUpdate = {
    token: data.token,
    endpoint: data.endpoint,
  };
  yukumo.handleVoiceServerUpdate(data.guildId, serverUpdate);
});
